...
WCS.Core.Parse=function(){(function(a){a.fn.textArray=function(){return this.map(function(){return a(this).text()}).get()}})(jQuery);var c={};$.xmlns.xlink="http://www.w3.org/1999/xlink";$.xmlns.ows="http://www.opengis.net/ows/2.0";$.xmlns.wcs="http://www.opengis.net/wcs/2.0";$.xmlns.gml="http://www.opengis.net/gml/3.2";$.xmlns.gmlcov="http://www.opengis.net/gmlcov/1.0";$.xmlns.swe="http://www.opengis.net/swe/2.0";var b=function(a,b){c.hasOwnProperty(a)?c[a].push(b):c[a]=[b]},a=function(a){for(var c in a)b(c,
a[c])},d={throwOnException:!1},e=function(a){for(var b=a.find("gml|Envelope"),c=a.find("gml|domainSet"),e=WCS.Util.stringToIntArray(c.find("gml|low").text()),d=WCS.Util.stringToIntArray(c.find("gml|high").text()),h=[],f=0;f<Math.min(e.length,d.length);++f)h.push(d[f]+1-e[f]);var g=[];c.find("gml|offsetVector").each(function(){g.push(WCS.Util.stringToFloatArray($(this).text()))});for(var k=[],f=0;f<g.length;++f)for(var i=0;i<g.length;++i)0!=g[i][f]&&k.push(g[i][f]);return{coverageId:a.find("wcs|CoverageId").text(),
dimensions:parseInt(a.find("gml|RectifiedGrid").attr("dimension")),bounds:{projection:b.attr("srsName"),lower:WCS.Util.stringToFloatArray(b.find("gml|lowerCorner").text()),upper:WCS.Util.stringToFloatArray(b.find("gml|upperCorner").text())},envelope:{low:e,high:d},size:h,origin:WCS.Util.stringToFloatArray(c.find("gml|pos").text()),offsetVectors:g,resolution:k,rangeType:a.find("swe|field").map(function(){var a=$(this);return{name:a.attr("name"),description:a.find("swe|description").text(),uom:a.find("swe|uom").attr("code"),
nilValues:a.find("swe|nilValue").map(function(){var a=$(this);return{value:parseInt(a.text()),reason:a.attr("reason")}}).get(),allowedValues:WCS.Util.stringToIntArray(a.find("swe|interval").text()),significantFigures:parseInt(a.find("swe|significantFigures").text())}}).get(),coverageSubtype:a.find("wcs|CoverageSubtype").text(),supportedCRSs:a.find("wcs|supportedCRS").textArray(),nativeCRS:a.find("wcs|nativeCRS").text(),supportedFormats:a.find("wcs|supportedFormat").textArray()}};a({Capabilities:function(a){var b=
a.find("ows|ServiceIdentification"),c=a.find("ows|ServiceProvider"),e=a.find("wcs|ServiceMetadata");return{serviceIdentification:{title:b.find("ows|Title").text(),"abstract":b.find("ows|Abstract").text(),keywords:b.find("ows|Keyword").textArray(),serviceType:b.find("ows|ServiceType").text(),serviceTypeVersion:b.find("ows|ServiceTypeVersion").text(),profiles:b.find("ows|Profile").textArray(),fees:b.find("ows|Fees").text(),accessConstraints:b.find("ows|AccessConstraints").text()},serviceProvider:{providerName:c.find("ows|ProviderName").text(),
providerSite:c.find("ows|ProviderSite").attr("xlink:href"),individualName:c.find("ows|IndividualName").text(),positionName:c.find("ows|PositionName").text(),contactInfo:{phone:{voice:c.find("ows|Phone ows|Voice").text(),facsimile:c.find("ows|Phone ows|Facsimile").text()},address:{deliveryPoint:c.find("ows|Address ows|DeliveryPoint").text(),city:c.find("ows|Address ows|City").text(),administrativeArea:c.find("ows|Address ows|AdministrativeArea").text(),postalCode:c.find("ows|Address ows|PostalCode").text(),
country:c.find("ows|Address ows|Country").text(),electronicMailAddress:c.find("ows|Address ows|ElectronicMailAddress").text()},onlineResource:c.find("ows|OnlineResource").attr("xlink:href"),hoursOfService:c.find("ows|HoursOfService").text(),contactInstructions:c.find("ows|ContactInstructions").text()},role:c.find("ows|Role").text()},serviceMetadata:{formatsSupported:e.find("wcs|formatSupported").textArray(),crssSupported:e.find("wcs|CrsMetadata wcs|crsSupported").textArray()},operations:$.makeArray(a.find("ows|OperationsMetadata ows|Operation").map(function(){var a=
$(this);return{name:a.attr("name"),getUrl:a.find("ows|Post").attr("xlink:href"),postUrl:a.find("ows|Get").attr("xlink:href")}})),contents:{coverages:a.find("wcs|Contents wcs|CoverageSummary").map(function(){var a=$(this);return{coverageId:a.find("wcs|CoverageId").text(),coverageSubtype:a.find("wcs|CoverageSubtype").text()}}).get()}}},ExceptionReport:function(a){a=a.find("ows|Exception");a={code:a.attr("exceptionCode"),locator:a.attr("locator"),text:a.find("ows|ExceptionText").text()};if(d.throwOnException)throw new Exception(a.text);
return a},CoverageDescriptions:function(a){return{coverageDescriptions:a.find("wcs|CoverageDescription").map(function(){return WCS.Core.Parse.callParseFunctions(this.localName,$(this))}).get()}},CoverageDescription:e,RectifiedGridCoverage:e});return{pushParseFunction:b,pushParseFunctions:a,parse:function(a){a="string"===typeof a?$.parseXML(a).documentElement:a.documentElement;return WCS.Core.Parse.callParseFunctions(a.localName,$(a))},callParseFunctions:function(a,b){if(c.hasOwnProperty(a)){for(var e=
c[a],d={},j=0;j<e.length;++j){var h=e[j](b);WCS.Util.deepMerge(d,h)}return d}throw Error("No parsing function for tag name '"+a+"' registered.");},options:d}}();namespace("WCS.EO");
