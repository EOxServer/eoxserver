    ...
    def handle(self, request):
        ...
        for layer_name in layers:
            for suffix in chain((None,), suffixes):
                ...
                raise "No such layer %s" % layer_name

            if iscollection(eo_object):
                ...
                def recursive_lookup(collection, suffix, used_ids, subsets):
                    ...
                    group = LayerGroup(eo_collection.identifier, suffix)
                    ...
                    for eo_object in eo_objects:
                        ...
                        if iscoverage(eo_object):
                            group.append(eo_object.cast())
                        elif iscollection(eo_object):
            ...
            elif iscoverage(eo_object):
                root_group.append(eo_object.cast())

        renderer = ms_component.get_renderer()


class LayerGroup(list):
    def __init__(self, name, suffix=None, iterable=None):
        self.name = name
        self.suffix = suffix
        if iterable:
    ...
    def walk(self, breadth_first=True):
        for item in self:
            try:
                ...
                yield (self.name,), self.suffix, eo_object
...
class WMS13GetMapDecoder(kvp.Decoder):
    layers = kvp.Parameter(type=typelist(","), num=1)
